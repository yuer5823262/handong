package com.example.dampouring.model.pojo;

public class SmartMixingPara {
    private Integer id;

    private Integer mbid;
    private String mbNo;
    private Double λtds;

    private Double g1;

    private Double g2;

    private Double g3;

    private Double g4;

    private Double g5;

    private Double g6;

    private Double g7;

    private Double g8;

    private Double g9;

    private Double c1;

    private Double c2;

    private Double c3;

    private Double c4;

    private Double c5;

    private Double c6;

    private Double c7;

    private Double c8;

    private Double c9;

    private Double p;

    private Double t;

    private Double v;

    private Double η;

    private Double t01; //出击口温度标准

    private Double tmins; //水温最小

    private Double tmaxs; //水温最大

    private Double gmaxb; //加冰量最大

    private Double qs;

    private Double qtds;

    private Double qds;

    private Double qzs;

    private Double qxs;

    public Integer getId() {
        return id;
    }

    public void setId(Integer id) {
        this.id = id;
    }

    public String getMbNo() {
        return mbNo;
    }

    public void setMbNo(String mbNo) {
        this.mbNo = mbNo;
    }

    public Integer getMbid() {
        return mbid;
    }

    public void setMbid(Integer mbid) {
        this.mbid = mbid;
    }

    public Double getΛtds() {
        return λtds;
    }

    public void setΛtds(Double λtds) {
        this.λtds = λtds;
    }

    public Double getG1() {
        return g1;
    }

    public void setG1(Double g1) {
        this.g1 = g1;
    }

    public Double getG2() {
        return g2;
    }

    public void setG2(Double g2) {
        this.g2 = g2;
    }

    public Double getG3() {
        return g3;
    }

    public void setG3(Double g3) {
        this.g3 = g3;
    }

    public Double getG4() {
        return g4;
    }

    public void setG4(Double g4) {
        this.g4 = g4;
    }

    public Double getG5() {
        return g5;
    }

    public void setG5(Double g5) {
        this.g5 = g5;
    }

    public Double getG6() {
        return g6;
    }

    public void setG6(Double g6) {
        this.g6 = g6;
    }

    public Double getG7() {
        return g7;
    }

    public void setG7(Double g7) {
        this.g7 = g7;
    }

    public Double getG8() {
        return g8;
    }

    public void setG8(Double g8) {
        this.g8 = g8;
    }

    public Double getG9() {
        return g9;
    }

    public void setG9(Double g9) {
        this.g9 = g9;
    }

    public Double getC1() {
        return c1;
    }

    public void setC1(Double c1) {
        this.c1 = c1;
    }

    public Double getC2() {
        return c2;
    }

    public void setC2(Double c2) {
        this.c2 = c2;
    }

    public Double getC3() {
        return c3;
    }

    public void setC3(Double c3) {
        this.c3 = c3;
    }

    public Double getC4() {
        return c4;
    }

    public void setC4(Double c4) {
        this.c4 = c4;
    }

    public Double getC5() {
        return c5;
    }

    public void setC5(Double c5) {
        this.c5 = c5;
    }

    public Double getC6() {
        return c6;
    }

    public void setC6(Double c6) {
        this.c6 = c6;
    }

    public Double getC7() {
        return c7;
    }

    public void setC7(Double c7) {
        this.c7 = c7;
    }

    public Double getC8() {
        return c8;
    }

    public void setC8(Double c8) {
        this.c8 = c8;
    }

    public Double getC9() {
        return c9;
    }

    public void setC9(Double c9) {
        this.c9 = c9;
    }

    public Double getP() {
        return p;
    }

    public void setP(Double p) {
        this.p = p;
    }

    public Double getT() {
        return t;
    }

    public void setT(Double t) {
        this.t = t;
    }

    public Double getV() {
        return v;
    }

    public void setV(Double v) {
        this.v = v;
    }

    public Double getΗ() {
        return η;
    }

    public void setΗ(Double η) {
        this.η = η;
    }

    public Double getT01() {
        return t01;
    }

    public void setT01(Double t01) {
        this.t01 = t01;
    }

    public Double getTmins() {
        return tmins;
    }

    public void setTmins(Double tmins) {
        this.tmins = tmins;
    }

    public Double getTmaxs() {
        return tmaxs;
    }

    public void setTmaxs(Double tmaxs) {
        this.tmaxs = tmaxs;
    }

    public Double getGmaxb() {
        return gmaxb;
    }

    public void setGmaxb(Double gmaxb) {
        this.gmaxb = gmaxb;
    }

    public Double getQs() {
        return qs;
    }

    public void setQs(Double qs) {
        this.qs = qs;
    }

    public Double getQtds() {
        return qtds;
    }

    public void setQtds(Double qtds) {
        this.qtds = qtds;
    }

    public Double getQds() {
        return qds;
    }

    public void setQds(Double qds) {
        this.qds = qds;
    }

    public Double getQzs() {
        return qzs;
    }

    public void setQzs(Double qzs) {
        this.qzs = qzs;
    }

    public Double getQxs() {
        return qxs;
    }

    public void setQxs(Double qxs) {
        this.qxs = qxs;
    }
}